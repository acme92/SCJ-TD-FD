...
#Sort filenames according to parameters
def sort_func(a, b):
	a = a.split("_")[0:-1]
	b = b.split("_")[0:-1]
...
def SCJTDFD(foldername):
	...
	ratio = 0			#Average ratio for true SCJ / inferred SCJ									
	iter_ratio = 0		#Ratio for ongoing iteration									
	iter_count = 0		#Iteration count (0 to 49)
	event_count = 0		#Location in Events array
	prob_count = 0		#Location in Probability array

	P0 = [[0 for i in range(1)] for j in range(10)]		#Arrays for storing ratio stats 
	P1 = [[0 for i in range(6)] for j in range(10)]		#Named according to length of duplication segment	
	P2 = [[0 for i in range(6)] for j in range(10)]
	P5 = [[0 for i in range(6)] for j in range(10)]
	...
	for filename in file_list:
		...
		if iter_count == 50:
			...
			if len_dup == 1:
				...
				P2[event_count][prob_count] = ratio/50		#Duplication segment length = 2	
			elif len_dup == 5:
				P5[event_count][prob_count] = ratio/50		#Duplication segment length = 5
			else:
				P10[event_count][prob_count] = ratio/50		#Duplication segment length = 10	
			iter_count = 0
			ratio = 0
			event_count += 1
	...
	plt.ylabel('SCJ true / SCJ inferred')
	plt.title('SCJ (inferred vs true) with single gene duplication')

	plt.figure(2)
	plt.plot(Events, P2)
	plt.xlabel('No. of events')
	plt.ylabel('SCJ true / SCJ inferred')
	plt.title('SCJ (inferred vs true) with two gene duplication')
	
	plt.figure(3)
	plt.plot(Events, P5)
	plt.xlabel('No. of events')
	plt.ylabel('SCJ true / SCJ inferred')
	plt.title('SCJ (inferred vs true) with five gene duplication')
